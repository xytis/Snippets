...
class FormatTestSuite : public Test::Suite
{
public:
  FormatTestSuite()
  {
	TEST_ADD(FormatTestSuite::format1);
	TEST_ADD(FormatTestSuite::format2);
  }
private:
  void format1()
  {
	Log::Format format("caller", "level");
	stringstream line;
	format.apply(&line);
	TEST_ASSERT(line.str() == "caller: level");
  }
  void format2()
  {
	Log::Format format("caller", "level");
...
//  ts.add(auto_ptr<Test::Suite>(new ThrowTestSuite));
	ts.add(auto_ptr<Test::Suite>(new FormatTestSuite));
